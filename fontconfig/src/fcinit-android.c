...
static FcConfig *
FcInitFallbackConfig(const char *fontDirPath)
{
    FcConfig	*config;
    ...
    if (!config)
        goto bail0;
    if (!FcConfigAddDir(config, (FcChar8 *)fontDirPath))
        goto bail1;

    // A font cache is just saved in the fontDirPath.
    if (!FcConfigAddCacheDir(config, (FcChar8 *)fontDirPath))
        goto bail1;
    return config;
...
FcInitLoadOwnConfig(FcConfig *config, const char *fontDirPath)
{
    ...
    {
        FcConfigDestroy(config);
        return FcInitFallbackConfig(fontDirPath);
    }
...
{
    config = FcInitLoadOwnConfig(config, fontDirPath);
    if (!config)
        return 0;
...
FcConfig *
FcInitLoadConfigAndFonts(const char *fontDirPath)
{
    return FcInitLoadOwnConfigAndFonts(NULL, fontDirPath);
}

FcBool
FcInit(const char *path)
{
    return FcConfigInit(path);
}
